...
int kmeans(Point *points, int points_counts, int clusters_count, int nb_t) {
  ...
  Cluster *local_clusters_array =
      calloc(nb_t * clusters_count, sizeof(Cluster));
  int converged = 0;
...
#pragma omp parallel num_threads(nb_t)
  {
    ...
    Cluster *local_clusters = &local_clusters_array[tid * clusters_count];
...
#pragma omp for
      for (int k = 0; k < clusters_count; k++) {
        local_clusters[k].point_acc.x = 0;
        local_clusters[k].point_acc.y = 0;
        local_clusters[k].point_acc.z = 0;
        local_clusters[k].points_count = 0;
      }
      // Implicit Barrier
      printf("[%d] %d : B\n", iter, tid);
...
#pragma omp for
      for (int i = 0; i < points_counts; i++) {
        ...
        local_clusters[nearest].point_acc.x += p.x;
        local_clusters[nearest].point_acc.y += p.y;
        local_clusters[nearest].point_acc.z += p.z;
        local_clusters[nearest].points_count++;
...
#pragma omp for
      for (int k = 0; k < clusters_count; k++) {
        ...
        for (int t = 0; t < nb_t; t++) {
          ...
          Cluster *src = &local_clusters_array[t * clusters_count + k];

          clusters[k].point_acc.x += src->point_acc.x;
          clusters[k].point_acc.y += src->point_acc.y;
          clusters[k].point_acc.z += src->point_acc.z;

          clusters[k].points_count += src->points_count;
        }
      } // Implicit Barrier
      printf("[%d] %d : D\n", iter, tid);
...
#pragma omp for reduction(+ : converged)
      for (int k = 0; k < clusters_count; k++) {
        int changed = !compute_barycenter(&clusters[k]);
        converged += changed;
      }
